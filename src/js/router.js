...
const getAction = (req, res) => {
    ...
    db.query(query, (err, result) => {
        ...
        else{ res.status(200).json(`Action №${req.params.id} doesn't exist`) }
    });
};
...
const addAction = (req, res) => {
    ...
    db.query(queryid, (err, result) => {
        ...
        db.query(querystate, (err, result) => {
            if (err) return res.status(500).json(err);
            if(!result.length) return res.status(200).json(`There is no id:${state_id} in state table`);
            ...
            db.query(queryactiontype, (err, result) => {
                if (err) return res.status(500).json(err);
                if(!result.length) return res.status(200).json(`There is no id:${actionType_id} in actionType table`);
...
const deleteAllActions = (req, res) => {
    ...
    db.query(query, (err, result) => {
        if (err) return res.status(500).json(err);
        res.status(200).json("All action were deleted");
    });
};
